...
var Vmap = function Vmap(dom, mallId, floorId, options) {

    if (!dom || !mallId || !floorId ||
        Object.prototype.toString.call(mallId) !== "[object String]" ||
    ...
    var publicSize = 25; //公共设施大小
    var fontSize = 20; //商铺字体的大小
    ...
    function drawUnitFont(scaleNum, color) {
        context.font = "500 15px Arial,Helvetica,'黑体'";
    ...
    //切换楼层 
    //添加更换楼层成功回调函数by owen
    function changeFloor(floorId,callback) {
    ...
    this.removeOverLayForMall =  function() {
        for (var i = 0; i < markers.length; i++) {
            $(markers[i].dom).remove();
        }
    }
